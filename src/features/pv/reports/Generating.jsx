...
export default function Generating({ values, onDone }) {
  ...
  useEffect(() => {
    ...
    // 2. Trigger the Python API call when it hits the last step ('Finalizing PDF')
    // if (i === steps.length - 1) {
    ...
    //         body: JSON.stringify({ values }) // Passes your React state parameters if needed by Python
    //       });

    //       if (!response.ok) throw new Error('Python generation failed');

    //       // PDF created successfully, transition into completion
    //       setI(prev => prev + 1);
    //     } catch (err) {
    //       console.error(err);
    //       setError("Failed to generate PDF. Make sure Python server is running.");
    //     }
    //   };
  ...
  return (
    <div style={{ flex: 1, display: 'grid', placeItems: 'center', background: 'var(--bg)' }}>
      <div style={{ width: 340, textAlign: 'center' }} className="fade-up">
        ...
        <div className="mono" style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 24 }}>{docNumber(values)}.docx</div>
